...
import { FaArrowRight, FaCalendarDays, FaChevronDown, FaCircleCheck, FaClock, FaFileLines, FaLightbulb, FaLocationDot, FaMagnifyingGlass, FaShieldHalved, FaWater } from 'react-icons/fa6'
...
export default function MyReports({ reports, loading, error, onBack, onReport }: Props) {
  ...
  const visible = useMemo(() => {
    ...
    return reports.filter(report => (filter === 'All' || report.status === filter) && (!query || [report.barangay, report.locationDetails, report.description].some(value => value.toLowerCase().includes(query))))
    ...
    <div className="resident-reports-layout">
      <section className="resident-reports-main" aria-label="Submitted reports">
        <div className="reports-toolbar">
          <div className="report-search"><FaMagnifyingGlass aria-hidden="true" /><input type="search" value={search} onChange={event => setSearch(event.target.value)} placeholder="Search barangay, location, or description" aria-label="Search your reports" /></div>
          <div className="status-filters" aria-label="Filter reports by status">{(['All', ...statuses] as const).map(status => <button type="button" key={status} aria-pressed={filter === status} className={filter === status ? 'selected' : ''} onClick={() => setFilter(status)}>{status === 'All' ? 'All Reports' : status}</button>)}</div>
          <label className="sort-control">Sort by<select value={sort} onChange={event => setSort(event.target.value)}><option value="newest">Newest first</option><option value="oldest">Oldest first</option></select></label>
        </div>
        {loading ? <div className="panel empty-reports" role="status">Loading your reports…</div> : error ? <p className="form-error" role="alert">{error}</p> : visible.length === 0 ? <div className="panel empty-reports"><span className="icon-disc"><FaFileLines /></span><h2>{reports.length ? 'No matching reports' : 'Your reports will appear here'}</h2><p>{reports.length ? 'Try another search or status.' : 'Submit your first flood report to track its progress from one place.'}</p><button className="button button-blue" onClick={reports.length ? () => { setFilter('All'); setSearch('') } : onReport}>{reports.length ? 'Show all reports' : 'Report a Flood'} <FaArrowRight /></button></div> : <div className="resident-report-list">
          <p className="sr-only" role="status">{visible.length} reports shown</p>
          {visible.map(report => {
            const currentIndex = statuses.indexOf(report.status)
            return <details className="panel resident-report" key={report.id}>
              <summary>
              ...
              <div className="report-expanded">
                <div className="report-detail-grid"><div><small>Barangay</small><strong>{report.barangay}</strong></div><div><small>Location</small><strong>{report.locationDetails}</strong></div><div><small>Severity</small><strong>{report.severity}</strong></div><div><small>Submitted</small><strong>{report.createdAt ? new Date(report.createdAt).toLocaleString('en-PH', { dateStyle: 'medium', timeStyle: 'short' }) : 'Saving…'}</strong></div></div>
                <h3>Incident details</h3><p className="report-description">{report.description}</p>
                <div className="report-timeline" aria-label={'Status timeline: ' + report.status}>{statuses.map((status, index) => <div key={status} className={index <= currentIndex ? 'complete' : ''}><span>{index < currentIndex ? <FaCircleCheck /> : index === currentIndex ? <FaClock /> : index + 1}</span><strong>{status}</strong></div>)}</div>
                <p className="small-note">Report reference: {report.id}</p>
                {report.photoPath && <ReportEvidence photoName={report.photoName} photoPath={report.photoPath} alt={'Full evidence photo for ' + report.locationDetails} />}
                {!report.photoPath && report.photoName && <p className="small-note">Legacy filename: {report.photoName}. No stored image is attached.</p>}
              </div>
            </details>
          })}
        </div>}
