...
export default function LocationPicker({ value, confirmed, disabled, onChange, onConfirm }: Props) {
  ...
  return <section className="location-picker" aria-labelledby="incident-map-title">
    <h3 id="incident-map-title">Incident location on map <span aria-hidden="true">*</span></h3>
    ...
    {value && <div className="location-selected"><p><strong>Selected pin:</strong> {coordinateLabel(value)}</p>{value.source === 'device' && <p>Device estimate: about ±{Math.ceil(value.accuracyMeters ?? 0)} metres. {Number(value.accuracyMeters) > 100 ? 'This is a broad estimate—adjust the pin before confirming.' : 'Check and adjust the pin if needed.'}</p>}<button type="button" className="text-link" disabled={disabled} onClick={() => { cancelRequest.current?.(); setLocating(false); onChange(null); onConfirm(false); setMessage('Pin removed. Choose the incident location before submitting.') }}>Clear selected pin</button></div>}
    <label className="location-confirm"><input type="checkbox" checked={confirmed} required disabled={disabled || !value || locating} onChange={event => onConfirm(event.target.checked)} /><span>I confirm that this pin marks the flood incident location.</span></label>
  </section>
}
