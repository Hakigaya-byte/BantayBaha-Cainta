...
test('picker starts without a pin, permission request, tiles, or prechecked confirmation', () => {
  ...
  for (const text of ['Use my current location', 'Choose on map', '(optional)', 'No background tracking', 'Enter coordinates manually']) assert.ok(html.includes(text))
...
test('a selected optional pin still requires confirmation and can be removed', () => {
  const html = render(Picker, { value: point, confirmed: false, disabled: false, onChange() {}, onConfirm() {} })
  assert.match(html, /Selected pin:/)
  assert.match(html, /type="checkbox" required=""/)
  assert.ok(!html.includes('checked=""'))
  assert.match(html, /Skip map \/ remove pin/)
})
